...
from as3lib import Error uint
from as3lib.flash.utils import ByteArray
from os import urandom


def generateRandomBytes(numberRandomBytes: uint):
   """
   ...
   """
   numberRandomBytes = uint(numberRandomBytes)
   if numberRandomBytes > 1024 or numberRandomBytes < 1:
      raise Error(f"generateRandomBytes; numberRandomBytes outside of expected range; expected uint between 1 and 1024, got {numberRandomBytes}")
   output = ByteArray()
   output.write(urandom(numberRandomBytes))
   return output
