...
class itkHTMLScrolledText(itkBaseWidget, tkhtmlview.HTMLScrolledText):
   ...
   def updateText(self):
      ...
      self.set_html(f'{temp[0]}<pre style="color: {self._fg}; background-color: {self._bg}; font-size: {cmath.resizefont(self._fontSize,self._window._mult)}px; font-family: {self._font}">{self._textCache}</pre>{temp[1]}')
      self["state"] = "disabled"
   def updateBackground(self):
      self['background'] = self._bg
      self.updateText()
...
class window: #! Make this a toplevel and get rid of children['root']
   def __init__(self, width, height, **kwargs):
      ...
      self.children["root"].geometry(f"{self._startwidth}x{self._startheight}")
      self.children["root"].title(self._title)
      ...
      if self._menu:
         if self._defaultMenu:
            ...
            self.menubar["root"] = tkinter.Menu(self.children["root"], bd=1)
            self.children["root"].config(menu=self.menubar["root"])
      self.children["display"] = itkDisplay(self.children["root"],itkWindow=self,background=self._color)
   ...
   def minimumSize(self,type_:str="b",**kwargs):
      ...
      if type_ == "w":
         if self._isMain:
            self.children["root"].minsize(kwargs["width"],int((kwargs["width"]*self._startheight)/self._startwidth) + 28)
         else:
            self.children["root"].minsize(kwargs["width"],int((kwargs["width"]*self._startheight)/self._startwidth))
      elif type_ == "h":
         if self._isMain:
            self.children["root"].minsize(int((self._startwidth*kwargs["height"])/self._startheght) - 52,kwargs["height"])
         else:
            self.children["root"].minsize(int((self._startwidth*kwargs["height"])/self._startheight),kwargs["height"])
      elif type_ == "b":
   ...
   def minimumSizeReset(self):
      if self._isMain:
         self.children["root"].minsize(262,int((262*self._startheight)/self_startwidth) + 28)
      else:
         self.children["root"].minsize(262,int((262*self._startheight)/self._startwidth))
   ...
   def _setFullscreen(self, value:bool):
      ...
      self.children["root"].attributes("-fullscreen", value)
   ...
   def aboutwin(self):
      if self.aboutwindow:
         if self.aboutwindow[0]:
            ...
            self.aboutwindow[2]["window"].configure(background=self._color)
            self.aboutwindow[2]["window"].bind("<Destroy>",self.closeabout)
            self.aboutwindow[2]["label"] = tkinter.Label(self.aboutwindow[2]["window"], font=("TkTextFont",9), justify="left", text=self.aboutwindow[1], background=self._color)
            self.aboutwindow[2]["label"].place(anchor="nw", x=7, y=9)
            self.aboutwindow[2]["okbutton"] = tkinter.Button(self.aboutwindow[2]["window"], text="OK", command=self.closeabout, background=self._color)
   ...
   def doResize(self, event):
      if event.widget == self.children["root"]:
         self._width, self._height = self.children["root"].winfo_width(), self.children["root"].winfo_height()
