...
class itkImageLabel(itkLabel):
   ...
   def updateText(self):
      self["image"] = self._window.images[self._imgname][3]
   def _getImgName(self):
      return self._imgname
   def _setImgName(self, name):
      if name in self._window.images:
         if self._imgname != '':
            self._window.images[self._imgname][0] -= 1
         self._imgname = name
         self["image"] = self._window.images[name][3]
         self._window.images[name][0] += 1
...
class window: #! Make this a toplevel and get rid of children['root']
   ...
   def __init__(self,width,height,title="Python",color="#FFFFFF",mainwindow:bool=True,defaultmenu:bool=True,nomenu:bool=False,dwidth=None,dheight=None,flashIcon=False):
      ...
      self.menubar = {}
      self.images = {"":[0,[0,0],"",""]} # imagename:[references=numReferences,osize=[width,height],oimage=data,rimage=data]
      if mainwindow == True:
   ...
   def addImage(self, image_name:str, image_data, size:tuple=None):
      ...
      self.images[image_name] = [0,[],image_data,""]
      if size != None:
         self.images[image_name][1] = [size[0],size[1]]
      else:
         ims = PIL.Image.open(BytesIO(image_data)).size
         self.images[image_name][1] = [ims[0],ims[1]]
      self.resizeImage((self.images[image_name][1][0],self.images[image_name][1][1]), image_name)
   ...
   def resizeImage(self, size:tuple, image_name):
      if image_name != "":
         img = PIL.Image.open(BytesIO(self.images[image_name][2]))
         img.thumbnail(size)
         self.images[image_name][3] = PIL.ImageTk.PhotoImage(img)
   def resizeChildren(self):
      nm = self.properties["nm"]
      for i in self.images:
         self.resizeImage((int(self.images[i][1][0]*nm),int(self.images[i][1][1]*nm)),i)
