...
class itkHTMLScrolledText(itkBaseWidget, tkhtmlview.HTMLScrolledText):
    ...
    def processText(self, text):
        ...
        Receives unformatted text as an arguement and returns the processed text.
    ...
    def text(self, text):
        if self.text != text:
            self._text = text
            self._textCache = self.processText(text)
        self.updateText()
...
class ComboEntryBox(itkBaseWidget, tkinter.Button):
    def __init__(self, master=None, **kwargs):
        ...
        self.frame = tkinter.Frame(master)
        if self._rows < 1:
            raise Error(f'[as3lib.interface_tk] ComboEntryBox; rows must be greater than or equal to 1, got {self._rows}')
...
class itkWindow(tkinter.Toplevel):
    ...
    Notes:
        ...
        tkinter.Tk. This window is managed automatically by
        flash.desktop.NativeApplication when using interface_tk.
    '''
    def _setIcon2(self, img):
        with PIL.Image.open(img) as image:
            self.iconphoto(True, PIL.ImageTk.PhotoImage(image))

    def _setIcon(self, fileorbytes):
        if isinstance(fileorbytes, bytes):
            # This doesn't need to stay open as far as I can tell
            with BytesIO(fileorbytes) as i:
                self._setIcon2(i)
    ...
    def __init__(self, **kwargs):
        ...
        if self._menu:
            if self._defaultMenu:
                ...
                self.menubar['helpmenu'].add_command(label='About as3lib', font=('Terminal', 8), command=partial(as3state.nativeApplication._aboutwindow.open, self))
...
if __name__ == '__main__':
    ...
    secondwindow = itkWindow(width=400, height=400, title='Second Window', main=False, menu=False)
